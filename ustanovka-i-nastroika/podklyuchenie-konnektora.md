...
После установки перейдите в приложение Олчат в левом меню портала:
...
К сканированию QR-кода также можно перейти из списка аккаунтов, не заходя в настройки коннектора, нажав на иконку «Отсканировать QR-код»:
...
После подключения коннектора в левом меню настроек коннектора, а также на главной странице со списком всех аккаунтов в столбце «Статус» вы увидите статус подключения и подключенный номер:
...
{% hint style="info" %}
Вы можете подключить виртуальный номер Битрикс24 к нашей системе. Для этого его необходимо подключить к WhatsApp на телефоне. Подключение происходит как с обычным номером, но подтверждение производится не через сообщение, а по звонку. После этого подключаете нашу интеграцию через сканирование QR-кода.
{% endhint %}
...
Игнорирование рекомендаций может привести к блокировке вашего аккаунта WhatsApp.
{% endhint %}
