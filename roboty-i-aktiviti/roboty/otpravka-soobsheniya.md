...
2. Введите **Номер телефона**, на который необходимо отправить сообщение<br>
   * [<sup>_**Важно:**_</sup>](#user-content-fn-1)[^1] <sup>убедитесь, что в роботе выбрано корректное поле с номером телефона. Если в карточке клиента заполнено поле «Мобильный телефон», а в настройке робота указано \{{Рабочий телефон\}}, робот не найдёт номер и сообщение не будет отправлено.</sup>\ <sup>Чтобы робот корректно подставлял номер из всех заполненных телефонных полей, рекомендуем в строке «Номер телефона» выбрать переменную \{{Телефон (текст)\}}. Указать её можно через меню «…» (три точки) напротив строки «Номер телефона».</sup>
