...
{% hint style="info" %}
Важно: убедитесь, что в роботе выбрано корректное поле с номером телефона. Если в карточке клиента заполнено поле «Мобильный телефон», а в настройке робота указано \{{Рабочий телефон\}}, робот не найдёт номер и сообщение не будет отправлено.
...
2. Выберите **Очередь отправки.** Настройка позволяет установить паузы между отправляемыми сообщениями. Паузы устанавливаются в настройках коннектора. Подробнее в статье [#intervaly-otpravki-soobshenii](../../ustanovka-i-nastroika/interfeisy-prilozheniya/opisanie-nastroek-konnektora.md#intervaly-otpravki-soobshenii "mention"). Доступны варианты:
