...
2.  Введите **Номер телефона**, на который необходимо отправить сообщение\
    [<sub>_**Важно:**_</sub>](#user-content-fn-1)[^1] <sub>_убедитесь, что в роботе выбрано корректное поле с номером телефона. Если в карточке клиента заполнено поле «Мобильный телефон», а в настройке робота указано \{{Рабочий телефон\}}, робот не найдёт номер и сообщение не будет отправлено._</sub>

    <sub>_Чтобы робот корректно подставлял номер из всех заполненных телефонных полей, рекомендуем в строке «Номер телефона» выбрать переменную \{{Телефон (текст)\}}. Указать её можно через меню «…» (три точки) напротив строки «Номер телефона»._</sub>
