...
{% hint style="warning" %}
Все действия робота «\[Олчат] \[ОЛ] Управление открытой линией»  выполняются от имени пользователя и для пользователя, который указан в роботе в поле **«Запускать от имени».**&#x20;

**Рекомендуем запускать данный робот от имени конкретного сотрудника.**&#x20;
{% endhint %}

Робот «\[Олчат] \[ОЛ] Управление открытой линией» позволяет выполнять следующие действия над открытой линией:
...
Для добавления робота перейдите в **Роботы ‒ Создать ‒ Выбор стадии ‒ Другие роботы ‒ \[Олчат] \[ОЛ] Управление открытой линией ‒ Добавить.**
